...
namespace SuperMap.Web.OGC
{
    ...
    public class GetWFSFeature : WFSServiceBase
    {
        ...
        internal override void ParseSuccessResult(XDocument document, string originResult, object userState)
        {
            ...
            OnProcessComplated(featureRes, originResult, userState);
        }

        private void ParseFeatureMember(GetWFSFeatureResult featureRes, XElement item)
        {
            if (item != null)
            {
                ...
                {
                    foreach(Feature f in GetFeatures(element, spatialPropName))
                    {
        ...
        private FeatureCollection GetFeatures(XElement ele, string spatialPropName)
        {
        ...
        /// <summary>${mapping_GetWFSFeatureService_Event_ProcessCompleted_D}</summary>
        public event EventHandler<ResultEventArgs<GetWFSFeatureResult>> ProcessCompleted;

        private void OnProcessComplated(GetWFSFeatureResult result, string originRes, object userSate)
        {
            if (ProcessCompleted != null)
            {
                ProcessCompleted(this, new ResultEventArgs<GetWFSFeatureResult>(result, originRes, userSate));
            }
        }

...
namespace SuperMap.WinRT.Utilities
{
    ...
    {
        public static double GetNearest(double targetRes, double zoomRes, double nextRes, double[] resolutions, double minRes, double maxRes)
        {
            if (resolutions == null || resolutions.Length == 0)
            {
                return Math.Abs(zoomRes - targetRes) < Math.Abs(zoomRes - nextRes) ? targetRes : nextRes;
            }
            int index;
            if (!DoubleUtil.ValueCheck(minRes) || !DoubleUtil.ValueCheck(maxRes))
            {
                index = GetNearestIndex(zoomRes, resolutions);
            }
            else
            {
                index = GetNearestIndex(zoomRes, resolutions, minRes, maxRes);
            }
        ...
        public static int GetNearestIndex(double resolution, double[] resolutions, double minRes, double maxRes)
        {
            ...
            if (DoubleUtil.ValueCheck(minRes) && DoubleUtil.ValueCheck(maxRes) && i != -1)
            {
